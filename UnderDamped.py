...
# Analytical solution
def Underdamped(V0,t):
    return V0/(omega*L)*np.exp((-R/(2*L))*t)*np.sin(omega*t)

t3 = [ti]
i_exect = [Underdamped(170,ti)]
dt = 0.000001
...
for i in range(10000):
    step = (170-150)/10000
    vol = 150+i*step
    err = abs(Underdamped(vol,0.0004820000000000052)-5)
    if err<error:
        best = vol
        error = err
...
for j in range(int((tf-ti)/dt)):
    t3.append(t3[j] + dt)
    i_exect.append(Underdamped(best,t3[-1])) 
...
for i in range(len(t1)):
    error_1 += (i1[i]-Underdamped(best,t1[i]-t3[x_offset]))**2
for i in range(len(t2)):
    error_2 += (i2[i]-Underdamped(best,t2[i]-t3[x_offset]))**2    
# print the error
