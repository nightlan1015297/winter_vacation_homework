...
# Analytical solution
def Criticaldamped(V0,t):
    return (V0*t/L)*np.exp((-R/(2*L))*t)
...
for i in range(10000):
    vol = 350+i*step
    err = abs(Criticaldamped(vol,0.0003160000000000012)-5)
    if err<error:
        best = vol
        error = err

t3 = [ti]
i_exect = [Criticaldamped(best,ti)]
dt = 0.000001

for j in range(int((tf-ti)/dt)):
    t3.append(t3[j] + dt)
    i_exect.append(Criticaldamped(best,t3[-1])) 
...
for i in range(len(t1)):
    error_1 += (i1[i]-Criticaldamped(best,t1[i]-t3[x_offset]))**2
for i in range(len(t2)):
    error_2 += (i2[i]-Criticaldamped(best,t2[i]-t3[x_offset]))**2    
# print the error
